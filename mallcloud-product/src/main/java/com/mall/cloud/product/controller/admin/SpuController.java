...
import com.mall.cloud.product.dto.SkuDTO;
import com.mall.cloud.product.dto.SpuDTO;
import com.mall.cloud.product.dto.SpuPageSearchDTO;
import com.mall.cloud.product.model.SpuExtension;
import com.mall.cloud.product.service.*;
import com.mall.cloud.api.product.vo.*;
...
public class SpuController {
    ...
    private void spuUpdateStatus(SpuDTO spu) {
        ...
        if (StrUtil.isNotBlank(error)) {
            throw new mallcloudException(error);
        }
    ...
    private void spuBatchUpdateStatus(SpuDTO spu) {
        ...
        if (CollUtil.isEmpty(spuList)) {
            throw new mallcloudException("您选择的商品信息有误，请刷新后重试");
        }
        ...
        if (CollUtil.isEmpty(spuIds)) {
            throw new mallcloudException("您所选择的商品中没有符合操作条件的商品");
        }
        spuService.batchRemoveSpuCacheBySpuId(spuIds);
        if (errorList.size() > 0) {
            throw new mallcloudException("商品id为：" + errorList.toString() + "的" + errorList.size() + "件商品不符合操作条件");
        }
        spuService.changeSpuStatus(spu.getSpuId(), spu.getStatus());
        spuService.removeSpuCacheBySpuId(spu.getSpuId());
    }
    ...
    private void checkSaveOrUpdateInfo(SpuDTO spuDTO) {
        if (!Objects.equals(Constant.PLATFORM_SHOP_ID, AuthUserContext.get().getTenantId()) && Objects.isNull(spuDTO.getShopCategoryId())) {
            throw new mallcloudException("店铺分类不能为空");
        }
        if (Objects.isNull(spuDTO.getCategoryId())) {
            throw new mallcloudException("平台分类不能为空");
        }
