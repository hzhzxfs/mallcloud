...
import com.mall.cloud.api.product.constant.CategoryLevel;
import com.mall.cloud.product.dto.CategoryDTO;
import com.mall.cloud.product.model.Category;
import com.mall.cloud.product.mapper.CategoryMapper;
...
//            throw new mallcloudException("分类不存在，请刷新后重试");
            return category;
        }
    ...
    public void update(Category category) {
        ...
        if (Objects.equals(dbCategory.getCategoryId(), category.getParentId())) {
            throw new mallcloudException("分类不能成为本身的上级分类");
        }
        existCategoryName(category);
    ...
    public void deleteById(Long categoryId) {
        ...
        if (count > 0) {
            throw new mallcloudException("该分类在使用中，不能进行删除操作");
        }
    ...
    private void existCategoryName(Category category) {
        ...
        if (countByName > 0) {
            throw new mallcloudException("分类名已存在，请重新输入");
        }
    }
}
